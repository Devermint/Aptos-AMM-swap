...
module swap::controller_tests {
    ...
    fun test_pause_should_fail_without_init(account: &signer) {
        controller::pause(account)
    }

    #[test(account = @swap)]
    #[expected_failure(abort_code = 308)]
    fun test_resume_should_fail_without_init(account: &signer) {
    ...
    fun test_pause_should_ok(
        swap: &signer, controller: &signer, beneficiary: address
    ) {
    ...
    fun test_not_controller_pause_should_fail(
        swap: &signer, controller: &signer, beneficiary: &signer
    ) {
        let controller_address = signer::address_of(controller);
        let beneficiary_address = signer::address_of(beneficiary);

        implements::initialize_swap_for_test(
            swap, controller_address, beneficiary_address
        );
